      ...
      {db.reports.map((r) => (
        <View key={r.id} style={styles.row}>
          ...
          <View style={styles.actions}>
            ...
            <Pressable style={[styles.actionBtn, { backgroundColor: "#0f172a" }]} onPress={() => removeContent(r.id, r.targetId, r.targetType)}>
    ...
    <View style={{ gap: 10 }}>
      {data.map((c) => (
        <View key={c.id} style={styles.row}>
          ...
          <Pressable style={[styles.actionBtn, { backgroundColor: "#f1f5f9" }]} onPress={() => dismiss.mutate(c.id)}>
            <Text style={[styles.actionBtnText, { color: "#334155" }]}>تجاهل</Text>
          </Pressable>
        </View>
      ))}
      ...
      {data.map((s) => (
        <View key={s.id} style={styles.rowStacked}>
          ...
          <View style={styles.rowStackedFooter}>
            ...
            <Pressable style={[styles.actionBtn, { backgroundColor: "#f1f5f9" }]} onPress={() => dismiss.mutate(s.id)}>
              <Text style={[styles.actionBtnText, { color: "#334155" }]}>تجاهل</Text>
            </Pressable>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  tabChip: { paddingVertical: 8, paddingHorizontal: 14, borderRadius: 999, backgroundColor: "#f1f5f9" },
  tabChipActive: { backgroundColor: "#0f172a" },
  tabChipText: { fontSize: 12, fontWeight: "800", color: "#334155" },
  ...
  typeBadge: { backgroundColor: "#eef2ff", borderRadius: 999, paddingVertical: 2, paddingHorizontal: 8 },
  typeBadgeText: { fontSize: 9.5, fontWeight: "900", color: "#4338ca" },
  rowStacked: { backgroundColor: "white", borderRadius: 14, padding: 12, borderWidth: 1, borderColor: "#eef1f6", gap: 8 },
  rowStackedFooter: { flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  suggestionText: { fontSize: 12.5, color: "#0f172a", lineHeight: 19 },
